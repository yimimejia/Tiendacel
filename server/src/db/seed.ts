...
const seedAdminEmail = process.env.SEED_ADMIN_EMAIL ?? 'admin@vibran.tech';
const seedAdminPassword = process.env.SEED_ADMIN_PASSWORD ?? 'VibranTech2026*';
...
async function runSeed() {
  ...
  const adminUser = await db
    ...
    .values({
      ...
      usernameOrEmail: seedAdminEmail,
      passwordHash,
      roleId: adminRole.id,
      branchId: null,
      isActive: true,
  ...
  const [admin] = adminUser.length
    ? adminUser
    : await db.select().from(users).where(eq(users.usernameOrEmail, seedAdminEmail)).limit(1);
...
runSeed()
  .then(() => {
    ...
    console.log(`Admin inicial: ${seedAdminEmail}`);
    console.log(`Password inicial: ${seedAdminPassword}`);
  })
  .catch((error) => {
